...
var fs = require("fs"),
	path = require("path"),
	builtEdge = path.resolve(
		__dirname,
		"../build/Release/" +
			(process.env.EDGE_USE_CORECLR ||
			!fs.existsSync(
				path.resolve(__dirname, "../build/Release/edge_nativeclr.node")
			)
				? "edge_coreclr.node"
				: "edge_nativeclr.node")
	),
...
if (process.env.EDGE_NATIVE) {
	...
	edgeNative = path.resolve(
		...
		"./native/" +
			...
			determineVersion() +
			"/" +
			(process.env.EDGE_USE_CORECLR ? "edge_coreclr" : "edge_nativeclr")
	);
} else {
	throw new Error(
		"The edge native module is not available at " +
			builtEdge +
...
exports.func = function (language, options) {
	...
	if (typeof options === "string") {
		...
		options = {
			source: options,
			jsFileName: stack[1].getFileName(),
			jsLineNumber: stack[1].getLineNumber(),
		};
	...
	if (typeof language !== "string") {
		...
		try {
			...
			throw new Error(
				"The '" +
					compilerName +
					"' module required to compile the '" +
					language +
					"' language " +
		...
		if (process.env.EDGE_USE_CORECLR) {
			options.bootstrapDependencyManifest =
				compiler.getBootstrapDependencyManifest();
		}
	}
	...
	if (options.assemblyFile) {
		if (!options.typeName) {
			...
			if (!matched) {
				throw new Error(
					"Unable to determine the namespace name based on assembly file name. " +
